...
void SImGuiWidget::Construct(const FArguments& InArgs)
{
	...
	// Force redraw widget every frame
	ForceVolatile(true);
...
void SImGuiWidget::OnDebugDraw()
{
	...
	if (ContextProxy && CVars::DebugInput.GetValueOnGameThread() > 0)
	{
		...
		if (ImGui::Begin("ImGui Input State", &bDebug))
		{
				...
				for (int32 Row = 0; Row < RowsNum; Row++)
				{
					for (int32 Col = 0; Col < 2; Col++)
					{
						...
						if (Idx < Num)
						{
							...
							const ImGuiKey ImGuiKey = ImGuiInterops::GetImGuiKey(Key);
							Styles::TextHighlight(true, [&]()
							{
								TwoColumns::Value(*Key.GetDisplayName().ToString(), ImGuiKey);
							});
						}
						else
						{
			...
			{
				const FIntPoint mousePos = InputState.GetMousePosition().IntPoint();
				TwoColumns::Value("Position X", mousePos.X);
				TwoColumns::Value("Position Y", mousePos.Y);
